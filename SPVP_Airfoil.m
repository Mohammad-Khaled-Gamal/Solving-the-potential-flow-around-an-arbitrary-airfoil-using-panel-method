...
NACA = '0006';                                                              % NACA airfoil to load [####(#)]
...
if (flagPlot(6) == 1)
    ...
    set(gcf,'Color','White');                                               % Set color to white
    set(gca,'FontSize',12);                                                 % Set font size
    contourf(XX,YY,CpXY,100,'EdgeColor','blue');                            % Plot Cp contour
    contourcbar
    fill(XB,YB,[0.75 0.75 0.75]);                                                        % Plot airfoil as black polygon
    xlabel('X Units');                                                      % Set X-label
    ylabel('Y Units');                                                      % Set Y-label
...
v_req(i)=Vt(v_index(i)-1)+((X_req(i)-XC(v_index(i)-1))/(XC(v_index(i))-XC(v_index(i)-1)))*(Vt(v_index(i))-Vt(v_index(i)-1));
end
